export function NewsOrgIcon() {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      x="0px"
      y="0px"
      viewBox="0 0 40 40"
      html-style="fill:#000000; z-index:1;"
    >
      <path
        fill="#dff0fe"
        d="M20,38.5C9.799,38.5,1.5,30.201,1.5,20S9.799,1.5,20,1.5S38.5,9.799,38.5,20S30.201,38.5,20,38.5z"
      ></path>
      <path
        fill="#4788c7"
        d="M20,2c9.925,0,18,8.075,18,18s-8.075,18-18,18S2,29.925,2,20S10.075,2,20,2 M20,1 C9.507,1,1,9.507,1,20s8.507,19,19,19s19-8.507,19-19S30.493,1,20,1L20,1z"
      ></path>
      <path
        fill="#4788c7"
        d="M3.061 12L37 12 35.977 11 4.083 11zM36.939 28L3 28 4.023 29 35.917 29z"
      ></path>
      <path
        fill="none"
        stroke="#4788c7"
        html-stroke-linecap="round"
        html-stroke-miterlimit="10"
        d="M31.417 12.5C29.468 6.029 25.088 1.5 20 1.5s-9.468 4.529-11.416 11M8.584 27.5c1.949 6.471 6.328 11 11.416 11s9.468-4.529 11.417-11"
      ></path>
      <path
        fill="none"
        stroke="#4788c7"
        html-stroke-linecap="round"
        html-stroke-miterlimit="10"
        d="M15.905 27.5c.767 6.73 2.499 11 4.095 11s3.328-4.27 4.095-11M24.095 12.5c-.767-6.73-2.499-11-4.095-11s-3.328 4.27-4.095 11"
      ></path>
      <g>
        <path
          fill="#4788c7"
          d="M12.289,16h1.593l-1.852,8h-1.75l-1.163-5.132C9.055,18.6,9.018,18.3,9.005,17.969h-0.02 c-0.029,0.364-0.072,0.664-0.127,0.898L7.665,24H5.843L4,16h1.725l0.987,5.328c0.043,0.227,0.073,0.532,0.093,0.915h0.029 c0.013-0.286,0.06-0.599,0.142-0.937L8.247,16h1.691l1.149,5.372c0.043,0.197,0.08,0.484,0.112,0.859h0.02 c0.013-0.294,0.047-0.591,0.103-0.893L12.289,16z"
        ></path>
      </g>
      <g>
        <path
          fill="#4788c7"
          d="M22.38,21.339c-0.055,0.301-0.089,0.599-0.103,0.893h-0.02c-0.033-0.375-0.07-0.662-0.112-0.859 L20.997,16h-1.691l-1.271,5.305c-0.082,0.339-0.128,0.651-0.142,0.937h-0.029c-0.02-0.383-0.05-0.688-0.093-0.915L16.785,16 h-1.725l1.843,8h1.823l1.193-5.132c0.055-0.234,0.098-0.534,0.127-0.898h0.02c0.013,0.331,0.05,0.63,0.112,0.898L21.339,24h1.75 l1.852-8h-1.593L22.38,21.339z"
        ></path>
      </g>
      <g>
        <path
          fill="#4788c7"
          d="M34.407,16l-0.968,5.339c-0.055,0.301-0.089,0.599-0.103,0.893h-0.02 c-0.033-0.375-0.07-0.662-0.112-0.859L32.056,16h-1.691l-1.271,5.305c-0.082,0.339-0.128,0.651-0.142,0.937h-0.029 c-0.02-0.383-0.05-0.688-0.093-0.915L27.843,16h-1.725l1.843,8h1.823l1.193-5.132c0.055-0.234,0.098-0.534,0.127-0.898h0.02 c0.013,0.331,0.05,0.63,0.112,0.898L32.398,24h1.75L36,16H34.407z"
        ></path>
      </g>
    </svg>
  );
}
